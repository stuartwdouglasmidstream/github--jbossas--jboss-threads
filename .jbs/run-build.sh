...
export PROJECT_VERSION=3.6.1.Final
...
if [ -n "" ]; then
  echo "Setting version to 3.6.1.Final to match enforced version"
  mvn -B -e -s "/root/software/settings/settings.xml" -t "/root/software/settings/toolchains.xml" org.codehaus.mojo:versions-maven-plugin:2.8.1:set -DnewVersion="3.6.1.Final" | tee /root/project/logs/enforce-version.log
fi
